...
impl TranslateProvider for KimiProvider {
    async fn translate(
        &self,
        ...
        let api_key = self
            .get_api_key()
            .ok_or_else(|| TranslateError::ApiKeyMissing("Kimi".to_string()))?;

        let api_url = "https://api.moonshot.cn/v1/chat/completions";
        ...
        let model = request.model.as_deref().unwrap_or("moonshot-v1-8k");
